...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        if not remaining_separators:
            # Fallback to character splitting if no separators left
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        def _stats(chunks_list: list[str]) -> dict:
            count = len(chunks_list)
            avg_length = sum(len(c) for c in chunks_list) / count if count > 0 else 0.0
            return {"count": count, "avg_length": avg_length, "chunks": chunks_list}
